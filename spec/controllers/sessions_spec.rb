...
describe "Sessions Controller", "index action" do
  ...
  it 'logins and redirects' do
    ...
    controller.session[:person].should_not be_nil
    controller.session[:person].should == @quentin.id
    controller.should redirect_to("/")
  end
   
  it 'fails login and does not redirect' do
    controller = post "/login", :login => 'quentin', :password => 'bad password'
    controller.session[:person].should be_nil
    controller.should be_successful
  end

  it 'logs out' do
    controller = get("/logout"){|controller| controller.stub!(:current_person).and_return(@quentin) }
    controller.session[:person].should be_nil
  ...
  it 'deletes token on logout' do
    controller = get("/logout") {|request| request.stub!(:current_person).and_return(@quentin) }
